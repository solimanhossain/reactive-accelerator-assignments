...
function getSingleMovie(id) {
    const singleMovie = movieList.results.find((movie) => movie.id === id);
    return singleMovie;
}

function editMovie(id, title) {
    const movieIndex = movieList.results.findIndex((movie) => movie.id === id);
    movieList.results[movieIndex].title = title;
    const movieEdit = movieList.results[movieIndex];
    return movieEdit;
}

function deleteMovie(id) {
    const movieIndex = movieList.results.findIndex((movie) => movie.id === id);
    const movieDeleted = movieList.results[movieIndex];
    movieList.results.splice(movieIndex, 1);
    return movieDeleted;
}

export { getMovies, getSingleMovie, deleteMovie, editMovie };
